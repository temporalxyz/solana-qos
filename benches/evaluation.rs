...
use qos_model::{
    models::{ip_signer::IpSignerModel, ip_signer_stake::IpSignerStakeModel},
    QoSTransactionMeta,
};
...
fn ip_signer(c: &mut Criterion) {
    ...
    let (model, ips, signers) = mock_ip_signer_model(100_000_000, 3_000, 10_000);
...
fn ip_signer_stake(c: &mut Criterion) {
    // Fetch mock ip signer model
    let (model, ips, signers) = mock_ip_signer_stake_model(100_000_000, 3_000, 10_000);
    let some_ip = choose(&ips);
    let some_signer = choose(&signers);
...
fn mock_ip_signer_model(
    ...
    num_ips: usize,
    num_signers: usize,
) -> (IpSignerModel, Vec<u32>, Vec<[u8; 32]>) {
    // We first generate some random transaction metas.
    // These don't need to be reflective of mainnet to benchmark evaluation.
...
fn mock_ip_signer_stake_model(
    ...
    num_ips: usize,
    num_signers: usize,
) -> (IpSignerStakeModel, Vec<u32>, Vec<[u8; 32]>) {
    // We first generate some random transaction metas.
    // These don't need to be reflective of mainnet to benchmark evaluation.
    let ips: Vec<u32> = (0..num_ips)
        .map(|_| rand::random::<u32>())
    ...
    for &ip in &ips {
        stake_lookup.insert(ip, 1_000_000 * (rand::random::<u64>() % 100));
    }
    let total_stake = 500_000 * 1_000_000_000;

    // Initialize model
    let mut model = IpSignerStakeModel::new([], [], stake_lookup.clone(), total_stake);
    model.update_model(
        transaction_metas,
        num_ips,
        num_signers,
