...
impl QoSModel for IpSignerModel {
...
impl IpSignerModel {
    ...
    pub fn prune(&mut self, num_ips: usize, num_signers: usize) {
        // Prune ips
        let ips_to_delete = self.ip_score_inverse.len().saturating_sub(num_ips);
        ...
        // Prune signers
        let signers_to_delete = self.signer_score_inverse.len().saturating_sub(num_signers);
        let start = self.signer_score_inverse.len() / 2 - signers_to_delete / 2;
        let end = start + signers_to_delete;
        for (_score, signer) in self.signer_score_inverse.drain(start..end) {
